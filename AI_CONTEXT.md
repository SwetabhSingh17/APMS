...
- `client/src/`: Frontend React application.
  - `/pages`: Route-level components (e.g. `team-management.tsx`, `manage-project.tsx`, `projects.tsx`, `student-topics.tsx`, etc.).
...
- `scripts/`: DB seeding, backup, restore, setup, reset, and verification test suites.
  ...
  - `verify_team_management_and_supervisor_fix.ts`: Automated test suite for team management CRUD, safe dissolution with account retention, and optional supervisor allotment (`npm run test:teams`).
...
- **Coordinator**: Can approve/reject topic proposals, oversee all projects, view department stats, manage student project teams (modify rosters, rename teams, safe team dissolution), and manually reassign supervisors to project teams with real-time faculty search. Can reset student and supervisor passwords to defaults, but is strictly prohibited from resetting or modifying Administrator or Coordinator accounts (`FORBIDDEN_TARGET_STAFF`).
- **Admin**: Has full system access, can perform destructive actions (DB resets), manage all users and roles, full team management and safe dissolution, reset any user's password, and reassign supervisors.
